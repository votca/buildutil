...
ADV                         $(cecho GREEN -D)$(cecho CYAN WITH_FFTW=OFF) $(cecho GREEN -D)$(cecho CYAN CMAKE_DISABLE_FIND_PACKAGE_GSL=ON) $(cecho GREEN -D)$(cecho CYAN CMAKE_DISABLE_FIND_PACKAGE_MKL=ON) $(cecho GREEN -D)$(cecho CYAN BUILD_MANPAGES=OFF) $(cecho GREEN -D)$(cecho CYAN WITH_GMX=OFF) $(cecho GREEN -D)$(cecho CYAN WITH_H5MD=OFF))
...
while [[ $# -gt 0 ]]; do
 ...
 case $1 in
  ...
  -D)
    ...
    shift 2;;
  --minimal)
    cmake_opts+=( --no-warn-unused-cli -DWITH_FFTW=OFF -DCMAKE_DISABLE_FIND_PACKAGE_GSL=ON -DCMAKE_DISABLE_FIND_PACKAGE_MKL=ON -DBUILD_MANPAGES=OFF -DWITH_GMX=OFF -DWITH_SQLITE3=OFF -DWITH_H5MD=OFF )
    shift;;
   --release)
    rel="$2"
